...
def get_probs_for_words(sent, w1, w2):
    ...
    for ids_w1, ids_w2 in itertools.zip_longest(w1_ids, w2_ids):
        tens = torch.LongTensor([input_ids + add_tok_w1, input_ids + add_tok_w2, input_ids + add_tok_w2]).to(device)
        with torch.no_grad():
            res = model(tens)[0]
            res = res[..., 0:model.config.vocab_size] # Restrict to the vocabulary only
            res = torch.nn.functional.log_softmax(res, dim=-1)
            #print(res.size())
            #print(res[0,-1,ids_w1])
            #print(res[1,-1,ids_w2])
            #print(res)
            #res = res[0, -1]
            #res = res[..., 0:model.config.vocab_size] # Restrict to the vocabulary only
            #res = torch.nn.functional.log_softmax(res, dim=-1)
        if ids_w1 is not None:
            #res = torch.nn.functional.log_softmax(res[0], dim=-1)
            #score_w1 = score_w1 + res[0, -1, ids_w1].item()
            score_w1 = score_w1 + res[0,-1,ids_w1].item()
        if ids_w2 is not None:
            #print(res[1])
            #res = torch.nn.functional.log_softmax(res[1], dim=-1)
            #print(res)
            #score_w2 = score_w2 + res[-1, ids_w2].item()
    ...
    # alternative to compute the score for w2
    # add_tok = []
    # score_w2 = 0
    # for ids in w2_ids:
